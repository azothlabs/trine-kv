...
use super::{
    DurabilityMode, Error, Result, StorageAppendBackend, StorageAppendObject, StorageCapabilities,
    ...
    StorageReadObject, StorageWalRewriteBackend, StorageWriterLeaseBackend, allocate_read_buffer,
    ensure_whole_object_read_len, native_file_objects_from_paths, paginate_storage_objects,
    usize_to_u64,
};
...
impl BrowserStorageBackend {
    ...
    async fn directory_handle(&self, path: &Path, create: bool) -> Result<Option<DirectoryHandle>> {
        let segments = opfs_path_segments(path)?;
    ...
    async fn file_handle(&self, path: &Path, create: bool) -> Result<Option<FileHandle>> {
        let Some((directory, name)) = self.parent_directory_and_name(path, create).await? else {
            return Ok(None);
        };
    ...
    async fn read_object_bytes_inner(&self, object: &StorageObjectId) -> Result<Option<Arc<[u8]>>> {
    ...
    async fn read_object_bytes_sync(&self, object: &StorageObjectId) -> Result<Option<Arc<[u8]>>> {
    ...
    async fn write_object_bytes(&self, object: &StorageObjectId, bytes: &[u8]) -> Result<()> {
        let Some((directory, name)) = self.parent_directory_and_name(object.path(), true).await?
        else {
...
impl StorageDirectoryListBackend for BrowserStorageBackend {
    fn list_directory_files(
        ...
        Box::pin(async move {
            ...
            let Some(directory) = self.directory_handle(directory_id.path(), false).await? else {
                return Ok(Vec::new());
            };
...
impl StorageManifestPublishBackend for BrowserStorageBackend {
    fn publish_manifest(
        ...
        Box::pin(async move {
            require_browser_manifest_object(&object)?;
            Self::capabilities_for_browser().require(StorageCapability::AtomicManifestPublish)?;
...
async fn acquire_browser_writer_lease(object: StorageObjectId) -> Result<BrowserWriterLease> {
    ...
    Reflect::set(&options, &JsValue::from_str("ifAvailable"), &JsValue::TRUE)
        .map_err(|error| map_js_value_error(&error, "configure browser writer lease options"))?;
...
impl BrowserSyncAccessHandle {
    async fn open(file: &FileSystemFileHandle, object: Option<StorageObjectId>) -> Result<Self> {
    ...
    fn flush_if_needed(&self, durability: DurabilityMode) -> Result<()> {
        ...
        if durability == DurabilityMode::Flush {
            self.handle
                .flush()
                .map_err(|error| map_js_value_error(&error, "flush browser OPFS sync handle"))?;
        }
        Ok(())
    }
}
