...
fn range_delete_hides_point_reads_and_scans_without_breaking_snapshots() {
    ...
    for (key, value) in [(b"a", b"a1"), (b"b", b"b1"), (b"c", b"c1"), (b"d", b"d1")] {
        bucket.put_sync(key, value).expect("write key");
    }
    ...
    assert_eq!(
        bucket.get_sync(b"a").expect("a survives"),
        Some(b"a1".to_vec())
    );
    assert_eq!(bucket.get_sync(b"b").expect("b hidden"), None);
    assert_eq!(bucket.get_sync(b"c").expect("c hidden"), None);
    assert_eq!(
        bucket.get_sync(b"d").expect("d survives"),
        Some(b"d1".to_vec())
    );
    assert_eq!(
...
fn range_delete_participates_in_prefix_scans() {
    ...
    assert_eq!(
        collect(bucket.prefix_sync(b"user:").expect("prefix after delete")),
...
fn same_batch_order_decides_range_delete_conflicts() {
    ...
    assert_eq!(
        bucket.get_sync(b"m").expect("later put survives"),
    ...
    assert_eq!(
        bucket.get_sync(b"n").expect("later range delete wins"),
        None
    );
}
