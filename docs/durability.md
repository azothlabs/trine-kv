...
- Browser persistence is async-only on `wasm32-unknown-unknown`. It accepts
  ...
  `navigator.storage.getDirectory()` instead of a `window` object.
  DedicatedWorker uses `FileSystemSyncAccessHandle` for file byte reads,
  writes, WAL append, manifest publish, and WAL rewrite, and calls `flush()`
  for Trine's `Flush` durability mode. Window and SharedWorker use the async
  OPFS writable-stream path because the File System API exposes synchronous
  access handles only in DedicatedWorker. The browser integration gate covers
  separate Window, DedicatedWorker, and SharedWorker database targets, plus
  DedicatedWorker sync-handle exclusivity and lightweight timing probes.
