...
| --- | --- | --- | --- | --- | --- |
| Length lookup | `TruePlatformAsync` / keep true async where supported | `ThreadPoolManagedAsync` / audit metadata path | `ThreadPoolManagedAsync` / audit Apple metadata APIs | `ThreadPoolManagedAsync` | `ThreadPoolManagedAsync` / stay fallback unless target grows support |
| Random read | `TruePlatformAsync` / keep true async | `PlatformNativeAsyncButPartial` / complete overlapped read path | `PlatformNativeAsyncButPartial` / complete DispatchIO read path plus remaining steps | FreeBSD/Solaris-family `PlatformNativeAsyncButPartial`; other Unix `ThreadPoolManagedAsync` | `ThreadPoolManagedAsync` |
...
| Temporary write plus rename publish | `TruePlatformAsync` / keep true async | `PlatformNativeAsyncButPartial` / audit write, sync, rename, and directory steps | `PlatformNativeAsyncButPartial` / complete DispatchIO write plus remaining publish steps | FreeBSD/Solaris-family `PlatformNativeAsyncButPartial`; other Unix `ThreadPoolManagedAsync` | `ThreadPoolManagedAsync` |
| Append open | `TruePlatformAsync` / keep true async | `ThreadPoolManagedAsync` / audit append handle semantics | `PlatformNativeAsyncButPartial` / complete open semantics | `ThreadPoolManagedAsync` | `ThreadPoolManagedAsync` |
| Append | `TruePlatformAsync` / keep true async | `PlatformNativeAsyncButPartial` / keep resource-ordered overlapped append | `PlatformNativeAsyncButPartial` / keep resource-ordered DispatchIO append | FreeBSD/Solaris-family `PlatformNativeAsyncButPartial`; other Unix `ThreadPoolManagedAsync` | `ThreadPoolManagedAsync` |
| Persist/fsync | `TruePlatformAsync` / keep true async when backend completion is real | `ThreadPoolManagedAsync` / audit flush completion semantics | `PlatformNativeAsyncButPartial` / audit remaining fsync/fcntl steps | FreeBSD/Solaris-family `PlatformNativeAsyncButPartial`; other Unix `ThreadPoolManagedAsync` | `ThreadPoolManagedAsync` |
| WAL rewrite | `TruePlatformAsync` / keep true async | `PlatformNativeAsyncButPartial` / audit complete rewrite and publish steps | `PlatformNativeAsyncButPartial` / complete DispatchIO write plus remaining publish steps | FreeBSD/Solaris-family `PlatformNativeAsyncButPartial`; other Unix `ThreadPoolManagedAsync` | `ThreadPoolManagedAsync` |
| Delete | `TruePlatformAsync` / keep true async | `ThreadPoolManagedAsync` / audit delete semantics | `ThreadPoolManagedAsync` / audit delete semantics | `ThreadPoolManagedAsync` | `ThreadPoolManagedAsync` |
| Directory create | `TruePlatformAsync` / keep true async | `ThreadPoolManagedAsync` / audit create-directory completion | `ThreadPoolManagedAsync` / audit create-directory completion | `ThreadPoolManagedAsync` | `ThreadPoolManagedAsync` |
| Directory sync | `TruePlatformAsync` / keep true async where supported | `ThreadPoolManagedAsync` / audit directory handle sync | `PlatformNativeAsyncButPartial` / audit remaining directory sync steps | FreeBSD/Solaris-family `PlatformNativeAsyncButPartial`; other Unix `ThreadPoolManagedAsync` | `ThreadPoolManagedAsync` |
...
| Writer lease | `ThreadPoolManagedAsync` / replace only with an audited async lock lifetime | `ThreadPoolManagedAsync` / audit lock/open semantics | `ThreadPoolManagedAsync` / audit lock semantics | `ThreadPoolManagedAsync` / audit lock semantics | `ThreadPoolManagedAsync` or `Unsupported` by target |
